...
export async function findGroups(req: Request, res: Response) {
  try {
    ...
    const groups = await GroupModel.find({
      $or: [{ members: { $in: [currentUser] } }, { admin: currentUser }],
    }).sort({ createdAt: -1 });
...
export async function addMemberToGroup(
  ...
  try {
    ...
    const isAlreadyMember = await (GroupModel as any).isMember(
      memberId,
      groupId
    );
...
export async function deleteallGroups(req: Request, res: Response) {
  try {
